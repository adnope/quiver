...
export function AnalyticsView() {
  ...
  // React Queries
  const { data, isFetching, isFetchingNextPage, isError, refetch, fetchNextPage, hasNextPage } =
    useInfiniteQuery({
      queryKey: [
        'analytics',
        activeSubTab,
        queryParams,
        activeSubTab === 'protocols' ? undefined : direction,
        apiBaseUrl,
        Boolean(apiKey),
      ],
    ...
    <section className="flex flex-col h-[calc(100svh-8rem)] overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--panel)]">
      ...
      <div className="flex min-h-12 flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button type="button" size="sm" onClick={refreshData}>
